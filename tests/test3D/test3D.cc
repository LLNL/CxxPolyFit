...
int main() {
  ...
  Polynomial quad(coeff, 3,2);
  int N = 10;
  
  for(double ii = -N; ii <= N; ++ii) {
    vector<double> xx;
    xx.push_back(ii);
    xx.push_back(ii);
    xx.push_back(ii);
  ...
  for(double ii = -N; ii <= N; ++ii) {
    ...
    xx.push_back(ii);
    xx.push_back(ii);
    double yy = quad.derivYZ(xx);
    if(abs(yy) < .0001) { yy = 0; }  //Just avoiding a common thing that kills tests on different platforms
    cout << ii << " " << ii << " " << ii << " " << yy << endl;
  }

  cout << "# XZ derivative" << endl;

  for(double ii = -10; ii <= N; ++ii) {
    vector<double> xx;
    xx.push_back(ii);
    xx.push_back(ii);
    xx.push_back(ii);
    double yy = quad.derivXZ(xx);
    if(abs(yy) < .0001) { yy = 0; }  //Just avoiding a common thing that kills tests on different platforms
    cout << ii << " " << ii << " " << ii << " " << yy << endl;
  }
